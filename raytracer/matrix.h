...
public:
    ...
    static void applyTranslation(Matrix4x4& m, Vector3& v)
    {
        m = m * Matrix4x4::Translation(v);
    }

    static void applyScale(Matrix4x4& m, Vector3& v)
    {
        m = m * Matrix4x4::Scale(v);
    }

    static void applyRotation(Matrix4x4& m, Vector3& v)
    {
        if(fabs(v.x) > 0.0f)
            m = m * Matrix4x4::RotationX(v.x);
        if(fabs(v.y) > 0.0f)
            m = m * Matrix4x4::RotationY(v.y);
        if(fabs(v.z) > 0.0f)
            m = m * Matrix4x4::RotationZ(v.z);
    }

    /**
     *  static method to transform a point
     *  @param m the transformation matrix
     *  @param newP the resultant point
     *  @param p the original point
     */
    static void transformPoint(Matrix4x4& m, Vector3& newP, Vector3& p);

    /**
     *  static method to transform a direction
     *  @param m the transformation matrix
     *  @param newP the resultant direction
     *  @param p the original direction
     */
    static void transformDirection(Matrix4x4& m, Vector3& newD, Vector3& d);
